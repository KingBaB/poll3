...
import com.briup.apps.poll.bean.extend.SurveyAndAnswersVM;
...
public class SurveyController {
	...
	public MsgResponse findSurveyByClazzId(long id){
		try {
			List<SurveyVM> list = surveyService.selectByClazzIdAndCheckPass(id);
	...
	public MsgResponse findAllSurveyVM(){
		try {
			List<SurveyVM> list = surveyService.findAllSuerveyVM();
			return MsgResponse.success("success", list);
		} catch (Exception e) {
			e.printStackTrace();
			return MsgResponse.error(e.getMessage());
